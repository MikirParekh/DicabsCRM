...
class MainPageRepository{
  ...
  Future<List<DashboardModel>>fetchDeshboard(String userCode,String salesCode) async {
    final url = Uri.parse('$baseUrl/GetActivity?userCode=$userCode&salesCode=$salesCode');
  ...
  Future<List<String>>fetchOpportunity() async{
    ...
    if(response.statusCode==200){
      ...
      throw Exception('Failed to load Opportunity Data:${response.reasonPhrase}');
    }
  }
  ...
  Future<Map<String,dynamic>>submitForm(AddActivityList addActivityList,String userCode,String salesCode) async{
  ...
  static Future<LocationResponse?> postLocation({
    ...
    try {
      print('🌐 Sending location => Code: $userCode, Lat: $latitude, Long: $longitude');
